...
- **One landing page = one target keyword only** — never dilute a page with multiple target keywords
- All supporting keywords go on separate dedicated pages
...
## FAQ Rules (non-negotiable)
Every FAQ must follow this exact structure:
1. **Question contains the exact match keyword** — e.g. "What is The Brain Song?" not "What is it?"
2. **First sentence of the answer IS the direct answer** — state it immediately, no preamble
3. **Then expand** with supporting detail, context, or value
4. Use `FAQPage` schema for every FAQ section — enables Bing Copilot AI summary eligibility

### FAQ Example Pattern:
```
Q: What is The Brain Song?
A: The Brain Song is a 12-minute daily audio program that uses Gamma brainwave 
   entrainment to support focus, memory, and mental clarity. [direct answer first]
   It was developed around research linking Gamma frequency exposure to BDNF 
   production... [expansion follows]
```
...
## Secondary Keywords — Use Separate Pages
When you have multiple keywords, each gets its own dedicated page/URL:
- Never stuff secondary keywords into the main landing page
- Each page targets one keyword in title, H1, URL, and first 100 words
- Internal link between pages using the exact keyword as anchor text
- This protects the main EMD page's focus while capturing long-tail traffic
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "{Exact match keyword in question}",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "{Direct answer first sentence. Then expansion.}"
      }
    }
  ]
}
